...
type SearchResult = (BackendProduct & { _type: 'product' }) | (BackendStore & { _type: 'store' });
...
const SearchResultsScreen = () => {
  ...
  const computeScore = useCallback((item: BackendProduct | BackendStore): number => {
    let score = 0;
    ...
    if ('productType' in item) {
      ...
      if (item.category?.some((cat) => cat.toLowerCase().includes(q))) score += 5;
      score += (item.rating || 0);
    } else {
      // For stores
      if (item.name?.toLowerCase().includes(q)) score += 10;
      if (item.storeCategory?.toLowerCase().includes(q)) score += 6;
      if (item.tags?.some((tag: string) => tag.toLowerCase().includes(q))) score += 3;
      score += (item.rating || 0);
    }
    return score;
  }, [query]);
  ...
  const handlePress = (item: SearchResult) => {
    if (item._type === 'product') {
      router.push({ pathname: '/(screens)/ProductScreen', params: { id: item.id } });
    } else {
      router.push({ pathname: '/(screens)/StoreHomepageScreen', params: { id: item.id } });
    }
  };
  ...
  const renderSearchBar = () => (
    <View style={styles.searchBarContainer}>
      ...
      <View style={styles.searchBarPill}>
        <Text style={styles.searchBarText} numberOfLines={1}>{query}</Text>
      </View>
  ...
  const renderTopResult = () => {
    ...
    const tags = isProduct ? topResult.category : [topResult.storeCategory, ...(topResult.tags || [])];
    const rating = topResult.rating || 0;
    const distance = '240m'; // TODO: Replace with real geo distance if available
    return (
      <TouchableOpacity style={styles.topCard} onPress={() => handlePress(topResult)}>
        <Image source={imageUrl ? { uri: imageUrl } : undefined} style={styles.topCardImage} resizeMode="cover" />
        <View style={styles.topCardContent}>
          ...
          <Text style={styles.topCardTags} numberOfLines={1}>{Array.isArray(tags) ? tags.filter(Boolean).join(' · ') : tags}</Text>
  ...
  const renderSecondaryItem = ({ item }: { item: SearchResult }) => {
    ...
    const tags = isProduct ? item.category : [item.storeCategory, ...(item.tags || [])];
    const rating = item.rating || 0;
    const distance = '240m'; // TODO: Replace with real geo distance if available
    return (
      <TouchableOpacity style={styles.secondaryCard} onPress={() => handlePress(item)}>
        <Image source={imageUrl ? { uri: imageUrl } : undefined} style={styles.secondaryCardImage} resizeMode="cover" />
        <View style={styles.secondaryCardContent}>
          ...
          <Text style={styles.secondaryCardTags} numberOfLines={1}>{Array.isArray(tags) ? tags.filter(Boolean).join(' · ') : tags}</Text>
          <View style={styles.topCardRow}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginLeft: 12 }}>
              ...
              <Text style={styles.secondaryCardRating}>{rating.toFixed(1)}</Text>
            </View>
          </View>
        </View>
      </TouchableOpacity>
      ...
      {loading ? (
        ...
        <>
          ...
          <FlatList
            ...
            ListEmptyComponent={<Text style={{ textAlign: 'center', marginTop: 16 }}>No similar results found.</Text>}
